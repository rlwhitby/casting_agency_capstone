...
class CapstoneTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Set the CONFIG_TYPE environment variable to test
        ...
        # Ref: https://blog.miguelgrinberg.com/post/the-flask-mega-tutorial-part-viii-followers  # noqa
    ...
    def tearDown(self):
        ...
        # The database and application_context are reset after each test.
        db.session.remove()
        db.drop_all()
    ...
    def test_get_actors_404(self):
        ...
        self.assertEqual(
            data["message"], "There are no actors in the database."
            )

    def test_get_actors_401(self):
        """Test 401 error behaviour for GET /actors endpoint

        The test_get_actors_401 test tries to call the endpoint without
        the required authorization header to validate that the expected
    ...
    def test_get_actors_405(self):
        ...
        - a json object with:
            - a "success" value of False, and
            - a "message" of "The method is not allowed for the
            requested URL.".
        """
        # add an actor
        auth_header = self.get_headers(self.casting_director)
    ...
    def test_get_actor_movies_404(self):
        ...
        - a json object with:
            - a "success" value of False, and
            - a "message" of "No actor with id "
            + str(INVALID_ACTOR_ID) +" exists"
        """

        auth_header = self.get_headers(self.casting_assistant)
        res = self.client().get(
            "/actor/" + str(INVALID_ACTOR_ID) + "/movies",
            headers=auth_header
        )
        ...
        self.assertEqual(
            data["message"],
            "No actor with id " + str(INVALID_ACTOR_ID) + " exists"
        )
    ...
    def test_add_actor_422(self):
        ...
        - a json object with:
            - a "success" value of False, and
            - a "message" of "The request was well-formed
    ...
    def test_add_actor_401(self):
        ...
        required authorization header to validate that the expected
        HTTP exception is raised.
    ...
    def test_delete_actor(self):
        """Test successful behaviour for the /actors/actor_id/delete endpoint
    ...
    def test_get_movies_404(self):
        """Test 404 error behaviour of GET /movies endpoint

        The test_get_movies_404 test uses the GET method and the correct
        authorisation to try and get all the avaliable movies from the
        test database. As no movies exist, a 404 error should be returned.

        The assert statements check that the endpoint returns:
        - a status code of 404
        - a json object with:
            - a "success" value of False, and
            - a "message" of "There are no movies in the database.".
        """
    ...
    def test_get_movies_401(self):
        ...
        required authorization header to validate that the expected
        HTTP exception is raised if the incorrect method is used to query
        the endpoint.
    ...
    def test_get_movie_actors(self):
        ...
        auth_header = self.get_headers(self.casting_assistant)
        res = self.client().get(
            "/movie/" + str(VALID_MOVIE_ID) + "/actors",
            headers=auth_header
        )

        # Used to get the movie title for testing assertion
    ...
    def test_get_movie_actors_404(self):
        ...
        - a json object with:
            - a "success" value of False, and
            - a "message" of "No movie with id "
            + str(INVALID_MOVIE_ID) +" exists".
        """

        auth_header = self.get_headers(self.casting_assistant)
        res = self.client().get(
            "/movie/" + str(INVALID_MOVIE_ID) + "/actors",
            headers=auth_header
        )
        ...
        self.assertEqual(
            data["message"],
            "No movie with id " + str(INVALID_MOVIE_ID) + " exists"
        )
    ...
    def test_add_movie_422(self):
        ...
        - a json object with:
            - a "success" value of False, and
            - a "message" of "The request was well-formed but was unable
    ...
    def test_delete_movie_404(self):
        ...
        auth_header = self.get_headers(self.executive_producer)
        res = self.client().delete(
            "/movies/" + str(INVALID_MOVIE_ID) + "/delete",
            headers=auth_header
        )
    ...
    def test_delete_movie_401(self):
        ...
        The test_delete_movie_401 test tries to delete a movie without the
        required authorization header to validate that the expected
        HTTP exception is raised.
    ...
    def test_delete_movie_403(self):
        ...
        The test_delete_movie_403 test tries to delete a movie without the
        required permission to validate that the expected HTTP exception is
        raised.
    ...
    def test_cast_actors_in_movies(self):
        """Test of successful behaviour of POST /movie/movie_id/actor/actor_id

        The assert statements check that the endpoint returns:
        - a status code of 200
        - a json object with:
            - a "success" value of True
            - a "movie" value of the movie title, and
            - a "actor cast" value of the actor's name.
        """
        # add an actor
        auth_header = self.get_headers(self.casting_director)
        res = self.client().post(
        ...
        # Get the movie and actor objects for testing assertions.
        movie = Movie.query.filter(Movie.id == VALID_MOVIE_ID).one_or_none()
    ...
    def test_cast_actors_in_movies_404_movie_not_found(self):
        """Test of 404 error behaviour of POST /movie/movie_id/actor/actor_id
        endpoint when the movie_id is invalid
        ...
        auth_header = self.get_headers(self.casting_director)
        res = self.client().post(
            "/movie/" + str(INVALID_MOVIE_ID) + "/actor/" + str(VALID_ACTOR_ID),  # noqa
            headers=auth_header,
        )
    ...
    def test_cast_actors_in_movies_404_actor_not_found(self):
        ...
        auth_header = self.get_headers(self.casting_director)
        res = self.client().post(
            "/movie/" + str(VALID_MOVIE_ID) + "/actor/" + str(INVALID_ACTOR_ID),  # noqa
            headers=auth_header,
        )
